-- 四週打卡紀錄。
--
-- 先支援第一週介面，資料表保留 week_no / day_no 供四週共用。
...
create table if not exists public.hc_health_checkin (
  ...
  week_no        smallint not null default 1 check (week_no between 1 and 4),
...
revoke all on table public.hc_health_checkin from anon;
